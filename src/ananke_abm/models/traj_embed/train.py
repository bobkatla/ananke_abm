...
class ScheduleDataset(torch.utils.data.Dataset):
    def __init__(self, activities_csv: str, T_minutes: int):
        ...
        self.T = float(T_minutes); self.seqs = []
...
class TrajEncoderGRU(nn.Module):
    def __init__(self, d_p:int, K_time_token:int=4, K_dur_token:int=4, m_latent:int=16):
        super().__init__()
        self.d_p = d_p; self.Kt = K_time_token; self.Kd = K_dur_token
    ...
    def forward(self, p_idx_pad, t_pad, d_pad, lengths, e_p):
        ...
        last = h_last.squeeze(0); r = torch.cat([last, mean], dim=-1)
        z = self.proj(r); z = torch.nn.functional.normalize(z, p=2, dim=-1)
        return z
...
def collate_fn(batch, purpose_to_idx):
    ...
    for seq in batch:
        ...
        t0 = [t for _,t,_ in seq]; dd = [d for _,_,d in seq]
...
def build_masks(purposes_df: pd.DataFrame, purposes_order: list):
    P = len(purposes_order); name_to_idx = {p:i for i,p in enumerate(purposes_order)}
    def parse_bool(x):
        ...
        if isinstance(x, str):
            ...
            if x in ("n","no","false","f","0"):
                return False
        return False
    open_allowed = torch.zeros(P, dtype=torch.bool); close_allowed = torch.zeros(P, dtype=torch.bool)
    home_idx = name_to_idx.get("Home", None)
    if "can_open_close_day" in purposes_df.columns:
        for _, r in purposes_df.iterrows():
            p = r["purpose"]
            if p not in name_to_idx: continue
            allowed = parse_bool(r["can_open_close_day"]) 
            open_allowed[name_to_idx[p]] = allowed; close_allowed[name_to_idx[p]] = allowed
    else:
        open_allowed[:] = True
        close_allowed[:] = True
...
def main():
    ...
    time_cfg = TimeConfig(); basis_cfg = BasisConfig(); quad_cfg = QuadratureConfig()
    pep_cfg = PurposeEmbeddingConfig(); dec_cfg = DecoderConfig()

    acts = pd.read_csv(args.activities_csv); purp = pd.read_csv(args.purposes_csv)
    ...
    tr_subset = torch.utils.data.Subset(ds, tr_idx); va_subset = torch.utils.data.Subset(ds, va_idx)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for p_pad, t_pad, d_pad, lengths in dl_tr:
            ...
            for b in range(B):
                L = lengths[b]; seq = []
                for i in range(L):
                    pid = int(p_pad[b,i].item()); pstr = purposes[pid]
            ...
            total = DecoderConfig.ce_weight*ce + DecoderConfig.emd_weight*emd + DecoderConfig.tv_weight*tv + DecoderConfig.durlen_weight*durlen
            # Update model parameters
            opt.zero_grad(); total.backward(); nn.utils.clip_grad_norm_(params, 1.0); opt.step()
            total_tr += float(total.item())
            

        enc.eval(); dec.eval(); pds.eval()
        with torch.no_grad():
            e_p = pds(); loglam = pds.lambda_log(t_q)
            total_va = 0.0; ce_v=emd_v=tv_v=0.0; n_batches=0
            for p_pad, t_pad, d_pad, lengths in dl_va:
                ...
                for b in range(B):
                    ...
                    segs.append(seq)
                y = rasterize_batch(segs, purpose_to_idx, t_q).to(args.device)
                ce_l = dec.ce_loss(q, y, w_q); emd_l = dec.emd1d_loss(q, y, w_q); tv_l = dec.tv_loss(q, w_q)
                mu_d = torch.tensor([priors[p].mu_d for p in purposes], dtype=torch.float32, device=args.device)
                sd_d = torch.tensor([priors[p].sigma_d for p in purposes], dtype=torch.float32, device=args.device)
                ...
                total_va += float(total_l.item()); ce_v += float(ce_l.item()); emd_v += float(emd_l.item()); tv_v += float(tv_l.item()); n_batches += 1
