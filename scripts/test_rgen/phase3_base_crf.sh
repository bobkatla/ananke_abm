...
uv run rgen_schedule sample-population \
  ...
  --num-samples 42817 \
  --outprefix src/output/new_rgen/runs/cnn_crf/samples_base_5min_crf \
  --decode-mode crf \
...
uv run rgen_schedule eval-population \
  --samples src/output/new_rgen/runs/cnn_crf/samples_base_5min_crf.npz \
  --samples-meta src/output/new_rgen/runs/cnn_crf/samples_base_5min_crf_meta.json \
