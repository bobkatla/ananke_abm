...
uv run rgen_schedule sample-population \
  ...
  --num-samples 42817 \
  --outprefix src/output/new_rgen/runs/pmd_crf/pmd_crf \
  --decode-mode crf \
...
uv run rgen_schedule eval-population \
  --samples src/output/new_rgen/runs/pmd_crf/pmd_crf.npz \
  --samples-meta src/output/new_rgen/runs/pmd_crf/pmd_crf_meta.json \
