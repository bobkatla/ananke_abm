...
uv run rgen_schedule sample-population \
  ...
  --num-samples 42817 \
  --outprefix src/output/new_rgen/runs/cnn_crf_rejection/cnn_crf_reject \
  --decode-mode crf \
...
uv run rgen_schedule eval-population \
  --samples src/output/new_rgen/runs/cnn_crf_rejection/cnn_crf_reject.npz \
  --samples-meta src/output/new_rgen/runs/cnn_crf_rejection/cnn_crf_reject_meta.json \
  --reference src/output/new_rgen/full_train_5min.npz \
  --out-json src/output/new_rgen/runs/cnn_crf_rejection/eval_crf_report.json
...
uv run rgen_schedule viz-population \
  ...
  --samples-meta src/output/new_rgen/runs/cnn_crf_rejection/cnn_crf_reject_meta.json \
  --reference src/output/new_rgen/full_train_5min.npz \
  --outdir src/output/new_rgen/runs/cnn_crf_rejection/plots_crf